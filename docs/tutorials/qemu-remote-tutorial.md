...
There are officially supported Podman machines with QEMU on macOS and WSL2 on Windows, but if one wants to use a manually configured QEMU VM in remote mode, this guide should be helpful.

This writing will focus on Windows because no official QEMU support is provided, but it should be relatively easy to adapt the following for macOS use if desired.
...
Download or prepare Podman Windows build with `gvproxy` included and install as default. Podman release for Windows could be obtained from the official releases https://github.com/containers/podman/releases

#### gvproxy

When using older Podman releases, where `gvproxy.exe` is missing from the installation directory,
