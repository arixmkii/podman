...
func connectToPodmanSocket(maxBackoffs int, backoff time.Duration, qemuPID int, errBuf *bytes.Buffer, path string) (conn net.Conn, dialErr error) {
...
func (v *MachineVM) Start(name string, opts machine.StartOptions) error {
	...
	conn, err = connectToPodmanSocket(maxBackoffs, defaultBackoff, cmd.Process.Pid, stderrBuf, v.ReadySocket.Path)
	if err != nil {
		return err
	}
	defer conn.Close()
